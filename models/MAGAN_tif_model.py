...
class MAGANTifModel(BaseModel):
    """ This class implements the MA-GAN model for confocal to STED resolution enhancement for the tif dataset.
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        """
        # parser.set_defaults(norm='batch', netG='ST-Net', dataset_mode='tif', netS='resnet_6blocks',
        #                     batch_size=8, crop_size=256, preprocess='crop_rotation',
        #                     name='TAGANTifLung_{}'.format(datetime.date.today()))
        # for Lung cancer dataset
        parser.set_defaults(norm='batch', netG='ST-Net', dataset_mode='tif', netS='resnet_6blocks', netD='pixel',
                            load_size=256,
                            batch_size=8, crop_size=128, preprocess='resize_and_crop',
                            name='TAGANTifLung_{}'.format(datetime.date.today()))
        if is_train:
            parser.set_defaults(pool_size=0, gan_mode='vanilla', niter=900, niter_decay=100, batch_size=32,
                                preprocess='crop_rotation', crop_size=128)
    ...
    def __init__(self, opt):
        ...
        self.netS = networks.define_S(1, opt.output_nc, opt.ngf, opt.netS, opt.norm, False, opt.init_type,
        ...
        if self.isTrain:
            ...
            # self.criterionSEG = torch.nn.SmoothL1Loss()
    ...
    def backward_G(self):
        ...
        # segmentation loss
        self.loss_S_fake = self.criterionSEG(self.seg_fB,
                                             self.seg_GT) * self.opt.lambda_seg  # loss between fake B segmentation and GT
    ...
    def backward_S(self):
        self.loss_S_real = self.criterionSEG(self.seg_GT,
                                             self.seg_rB) * self.opt.lambda_seg  # loss between real B segmentation and GT
