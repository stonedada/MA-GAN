...
class MAGANAxonalRingsModel(BaseModel):
    """ This class implements the MA-GAN model for confocal to STED resolution enhancement for the axonal rings dataset.
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        """
        #  need to delete preprocess='crop_rotation', batch_size=8, crop_size=128 when test make sure have the same images
        parser.set_defaults(norm='batch', netG='resnet_9blocks', dataset_mode='axons', netS='resnet_6blocks',  name='TAGANAxonalFactin_{}'.format(datetime.date.today()))
        if is_train:
